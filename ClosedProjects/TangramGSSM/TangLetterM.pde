 ...
 */
public class TangLetterM extends TangShape {
  /**
   ...
   */
  public TangLetterM(float x, float y, int aShakeWeight, int unit, float scale, float xDiff, float yDiff, boolean alwaysShake, float rotationDiff, boolean flashAOn, int red, int green, int blue, int alpha) {
    this.x = x;
    this.y = y;
  ...
  public void drawThisTangram() {
    ...
    isHovered = drawTangramM(x, y, shakeWeight, unit, scale, alwaysShake, rotation, flashAOn, r, g, b, a);
  }
}
/**
 ...
 * 
 */
public boolean drawTangramM(float x, float y, int shakeWeight, int unit, float scale, boolean shakeAlwaysOn, float rotation, boolean flashAOn, int r, int g, int b, int a) {
  ...
  float origX = x - dim.larTriLeg();  //X position for the top of the tangram (X IS CENTER OF THE M!)
  ...
  TriL triL1 = new TriL(origX - dim.larTriLeg(), origY + dim.larTriLeg(), -90, unit);
  Square square = new Square(origX - dim.larTriLeg(), origY + dim.larTriLeg(), 0, unit);
  TriS triS1 = new TriS(origX - dim.larTriLeg(), origY + dim.larTriLeg() + dim.squareHeight(), 0, unit);
  TriM triM1 = new TriM(origX, origY + dim.larTriLeg() + dim.medTriHeight(), -135, unit);
  TriL triL2 = new TriL(origX + dim.larTriLeg(), origY + dim.larTriLeg(), -180, unit);
  TriS triS2 = new TriS(origX + dim.larTriLeg(), origY + dim.larTriLeg(), 90, unit);
  Parallelogram parallelogram = new Parallelogram(origX + dim.smaTriLeg(), origY + dim.larTriLeg(), 45, unit);
