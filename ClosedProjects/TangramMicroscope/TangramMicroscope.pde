...
void draw() {
  ...
  //SIZING VARIABLES
  int unit = 70;          //unit size for triangles
  ...
  //three MAIN tangrams!
  boolean microscopeIsHovered = drawTangramMicroscope((unit*2.5), (unit * 2.0/3.0) + unit * (-sqrt(2) + 2.5), shakeWeight, unit, scale, false, 0, mainFlashA, mainTangR, mainTangG, mainTangB, mainTangA);
  ...
  if (microscopeIsHovered && frameCount % 3 == 0) {
    //create a new spawn at the mouse location
    if (!drawBrock) {
      if (microscopeIsHovered) {
        TangMicroscope temp = new TangMicroscope(mouseX, mouseY, spawnShakeWeight, firstSpawnSize, scale, random(initMoveSize)-(initMoveSize/2.0), random(initMoveSize)-(initMoveSize/2.0), true, rotationDiff, spawnFlashAOn, spawnTangR, spawnTangG, spawnTangB, spawnTangA);
