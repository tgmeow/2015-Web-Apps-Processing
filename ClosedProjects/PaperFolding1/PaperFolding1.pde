...
void draw() {
  ...
  Point pointInside = new Point((mouseX - width*.5 - zoomTransX - panXY.x)/(zoom/100), (mouseY - height*.5 - zoomTransY - panXY.y)/(zoom/100));
 
 //recalculate screen edges for scrolling zooming purposes.
 float newScreenLeft = (-width*.5 - zoomTransX - panXY.x)/(zoom/100);
 float newScreenRight = (width*.5 - zoomTransX - panXY.x)/(zoom/100);
 //Draw this from 0 radians to 360 radians and makes a cool spotlight effect
  for(int rad = 0; rad<360; rad+=2){
      //Point on the circle
     Point circlePoint = new Point(cos(rad)*circleSize*.5,sin(rad)*circleSize*.5);
     //Midpoint
     Point midPoint = new Point((circlePoint.x+pointInside.x)*.5, (circlePoint.y+pointInside.y)*.5);
