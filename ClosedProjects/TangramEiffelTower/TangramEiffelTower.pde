...
PImage brock;  //global so that the image only has to be loaded from HD ONCE
...
void draw() {
  ...
  boolean drawBrock = false;    //Dr. Brockman!!!
  //SIZING VARIABLES
  int unit = 40;          //unit size for triangles
  ...
  //three MAIN tangrams!
  boolean towerIsHovered = drawTangramEiffelTower(width/2.0, (unit * 2.0/3.0) + unit * (-sqrt(2) + 2.5), shakeWeight, unit, scale, false, 0, mainFlashA, mainTangR, mainTangG, mainTangB, mainTangA);
  ...
  int spawnShakeWeight = 0;    //how hard to shake the spawns
  //COLOR OF SPAWNS
  int spawnTangR = 30;
  int spawnTangG = 30;
  int spawnTangB = 30;
  ...
  if (towerIsHovered && frameCount % 3 == 0) {
    //create a new spawn at the mouse location
    if (!drawBrock) {
      if (towerIsHovered) {
        TangEiffelTower temp = new TangEiffelTower(mouseX, mouseY, spawnShakeWeight, firstSpawnSize, scale, random(initMoveSize)-(initMoveSize/2.0), random(initMoveSize)-(initMoveSize/2.0), true, rotationDiff, spawnFlashAOn, spawnTangR, spawnTangG, spawnTangB, spawnTangA);
  ...
  for (int index = 0; index < miniSpawns.size(); index++) {
    ...
    if (aMiniTang.isHovered() && aMiniTang.getUnitSize() >= 1 && frameCount % 3 == 0) {
      ...
      if (!drawBrock) {
        ...
        if (aMiniTang instanceof TangMicroscope) {
          TangMicroscope temp = new TangMicroscope(mouseX, mouseY, spawnShakeWeight, int(aMiniTang.getUnitSize()/1.5), scale, random(initMoveSize)-(initMoveSize/2.0), random(initMoveSize)-(initMoveSize/2.0), true, rotationDiff, spawnFlashAOn, spawnTangR, spawnTangG, spawnTangB, spawnTangA);
          miniSpawns.add(temp);  //add the miniSpawn to the arraylist
        }
        if (aMiniTang instanceof TangEiffelTower) {
          TangEiffelTower temp = new TangEiffelTower(mouseX, mouseY, spawnShakeWeight, int(aMiniTang.getUnitSize()/1.5), scale, random(initMoveSize)-(initMoveSize/2.0), random(initMoveSize)-(initMoveSize/2.0), true, rotationDiff, spawnFlashAOn, spawnTangR, spawnTangG, spawnTangB, spawnTangA);
          miniSpawns.add(temp);  //add the miniSpawn to the arraylist
        }
      } else if (drawBrock  && frameCount % 5 == 0) {
   ...
   aMiniTang.drawThisTangram();  //draw the miniSpawn on the screen  
    ...
    if (aMiniTang.isOffScreen()) miniSpawns.remove(index);
  }
