...
void setup() {
  ...
  new Thread() {
    public void run() {
      photosphere = loadImage("E:\\Phone\\DCIM\\Camera\\PANO_20160608_082358.jpg");
      if(photosphere != null) {
        float r = float(photosphere.height)/float(photosphere.width);
        photosphere.resize(3000, int(8125*r));
      }
    }
  }.start();
}
