public class ButtonController extends Controller {
  ...
  public ButtonController(String name, String buttonLabel, Point menuXY) {
    ...
    button = new PolygonButton(controllerX + slidingWidth, controllerY + BUTTON_PADDING, slidingWidth, controllerHeight - BUTTON_PADDING*2);
  }
  ...
  private boolean slidingWidthContains(int clickX, int clickY) {
    float slidingWidth = controllerX + controllerWidth*2.0/5.0;
  ...
  public void drawController(int controllerX, int controllerY, int controllerWidth, int controllerHeight) {
    ...
    //Hides Text Overflow
    fill(BACKGROUND_COLOR);
    rect(controllerX + controllerWidth*2.0/5.0, controllerY, controllerWidth*3.0/5.0, controllerHeight);
    ...
    button = new PolygonButton(controllerX + int(slidingWidth), controllerY + BUTTON_PADDING, int(slidingWidth), controllerHeight - BUTTON_PADDING*2);
    button.drawButton();
...
//    text("" + this.peekValue(), controllerWidth*4.2/5.0, controllerY + controllerHeight/2.0 + TEXT_SIZE/4.0);
    ...
    fill(TEXT_COLOR);
    text(this.getLabel(), controllerX + 1.5*int(slidingWidth) - 0.5*textWidth(this.getLabel()), controllerY + controllerHeight/2.0 + TEXT_SIZE/4.0);
  }
}